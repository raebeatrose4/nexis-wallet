...
import { Nexis Network } from '~/utility/web3';
...
describe('account sagas', () => {
  ...
  Nexis Network.mnemonicToKeys = jest
    .fn()
    .mockImplementation((mnemonic: string) => ({
      publicAddress: `${mnemonic}-0xFF`,
    }));
  Nexis Network.getKeystore = jest.fn().mockImplementation(() => ({}));
  Nexis Network.getBalance = jest.fn().mockImplementation(() => '100000000000000000');
  Nexis Network.getPrivateKey = jest
    .fn()
    .mockImplementation((keystore, password) => `PRIVATE-${password}`);
  Nexis Network.isConnected = jest
    .fn()
    .mockImplementation(() => Promise.resolve(true));

  Nexis Network.setProvider(new FakeProvider());
  ...
  describe('sendFunds', () => {
    ...
    Nexis Network.transfer = jest.fn().mockImplementation(() => {
      return true;
    });

    if (!Nexis Network.web3) {
      throw new Error('web3 undefined');
    }

    Nexis Network.web3.eth.getGasPrice = jest
      .fn()
      .mockImplementation(() => '1');

    Nexis Network.web3.eth.estimateGas = jest.fn().mockImplementation(() => 1);
    ...
    it('sendFunds sending funds', async done => {
      await expectSaga(ACCOUNT_SAGAS.sendFunds, accountSendFunds(data))
        .withState(initial)
        .call.like({ fn: Nexis Network.getPrivateKey })
        .call.like({ fn: Nexis Network.transfer })
        .put.like({
          action: {
