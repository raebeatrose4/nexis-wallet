...
import * as NexisNetwork from '../../web3-functions'
...
function* createSetCredentials({
  ...
  const { privateKey, publicAddress } = yield call(NexisNetwork.mnemonicToKeys, mnemonic);;
  const pass = create.password || '';

  const keystore = NexisNetwork.getKeystore(privateKey, pass);
...
function* createSetConfirm() {
  ...
  let pKey = privateKey || '';
  if (!privateKey) {
    const keys = yield call(NexisNetwork.mnemonicToKeys, mnemonic);;
    pKey = keys.privateKey;
  }

  const keystore = NexisNetwork.getKeystore(pKey, password);
...
function* createSetRestoreCredentials({
  ...
  try {
    ...
    if (selectAccount && selectedAccount.privateKey) {
      const keys = NexisNetwork.privateKeyToKeys(selectedAccount.privateKey);
...
function* createRestoreMnemonics({
  ...
  const { publicAddress } = yield call(NexisNetwork.mnemonicToKeys, mnemonic);;
...
function* createRestorePrivateKey({
  ...
  const { publicAddress } = NexisNetwork.privateKeyToKeys(privateKey || '');
...
function* getPrivateKey({
  ...
  const { privateKey } = yield call(NexisNetwork.mnemonicToKeys, mnemonic);;
  cb(privateKey);
  // return privateKey
}
...
function* sendFunds({
  ...
  const privateKey = yield call(
    [NexisNetwork, NexisNetwork.getPrivateKey],
    keystore,
    password
  );

  const fee: string = yield call([NexisNetwork, NexisNetwork.estimateFee], {
    from,
    to,
  ...
  try {
    ...
    yield call([NexisNetwork, NexisNetwork.transfer], {
      from,
      to,
      value: amount.toString(),
      memo: message,
...
function* getFee({ gasLimit, cb }: ReturnType<typeof accountGetTransferFee>) {
  ...
  try {
    // const fee: string = yield call(NexisNetwork.estimateFee, {
    //   gasLimit,
    // });
    const fee = yield NexisNetwork.getTransactionFee(gasLimit);
    if (cb) {
      cb(fee);
    }
  } catch (e) {
...
function* uploadKeystore({
  ...
  try {
    ...
    const result = NexisNetwork.validateKeystore(keystore, password);
...
function* connectToNewProvider(current_node: number) {
  ...
  const { connected, timeout } = yield race({
    //@ts-ignore
    connected: call([NexisNetwork, NexisNetwork.setProvider], current.address),
    timeout: delay(10000),
  });
...
function* testConnectionSaga() {
  try {
    ...
    const { connected } = yield race({
      connected: call([NexisNetwork, NexisNetwork.isConnected]),
      timeout: delay(10000),
    });
