...
import { delegateStake, delegateUnstake, withdrawDelegateAmount }  from '../../web3-functions'
...
function* delegateByAddressSaga({
  ...
  try {
    const { pendingRewards, data } = yield NexisNetwork.getDelegationPendingRewards(
      publicKey,
      publicKey
    );
